...
// CleanPath removes any empty nodes in the given path, all the way to the root treeNode.
func CleanPath(treeNode interface{}, path util.Path) {
	if len(path) == 0 || treeNode == nil {
		return
	}
	switch nt := treeNode.(type) {
	// FIXME: is there a way not to duplicate these case statements?
	case map[string]interface{}:
		child := nt[path[0]]
		if child == nil {
			return
...
// isEmptyNode reports whether the tree node is empty i.e. an empty map or zero length slice.
func isEmptyNode(treeNode interface{}) bool {
	switch nt := treeNode.(type) {
	case map[string]interface{}:
		return len(nt) == 0
	case []interface{}:
