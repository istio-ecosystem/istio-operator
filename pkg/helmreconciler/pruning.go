...
func (h *HelmReconciler) Prune(all bool, objMap map[string]bool) error {
	allErrors := []error{}
	...
	if targetNamespace == "" {
		targetNamespace, _ = os.LookupEnv("WATCH_NAMESPACE")
	}
	err := h.PruneResources(namespacedResources, all, targetNamespace, objMap)
	if err != nil {
		allErrors = append(allErrors, err)
	}
	err = h.PruneResources(nonNamespacedResources, all, "", objMap)
	if err != nil {
		allErrors = append(allErrors, err)
	}
...
// resources.
func (h *HelmReconciler) PruneResources(gvks []schema.GroupVersionKind, all bool, namespace string,
	objMap map[string]bool) error {
	allErrors := []error{}
	...
	objectLoop:
		for _, object := range objects.Items {
			...
			key := util.GenerateKeyFromUnstructured(&object)
			// Delete un-existed components according to current manifest
			if !all && (objMap[key] || gvk.Kind == "Endpoints") {
