...
func installPreCheck(v *verifyInstallArgs, l *logger) error {
	l.print("\n")
	l.print("Checking the cluster to make sure it is ready for Istio installation...\n")
	l.print("\n")
	l.print("#1. Kubernetes-api\n")
	l.print("-----------------------\n")
	var errs error
	...
	if err != nil {
		...
		l.logAndPrintf("Failed to initialize the Kubernetes client: %v.\n", err)
	} else {
		l.print("Can initialize the Kubernetes client.\n")
	}
	version, err := c.serverVersion()
	if err != nil {
		errs = multierror.Append(errs, fmt.Errorf("failed to query the Kubernetes API Server: %v", err))
		l.logAndPrintf("Failed to query the Kubernetes API Server: %v.\n", err)
		l.logAndPrintf("Istio install NOT verified because the cluster is unreachable.\n")
		return errs
	}
	l.print("Can query the Kubernetes API Server.\n")

	l.print("\n")
	l.print("#2. Kubernetes-version\n")
	l.print("-----------------------\n")
	...
	if err != nil {
		...
		l.logAndPrintf("Istio is compatible with Kubernetes: %v.\n", v)
	}

	l.print("\n")
	l.print("#3. Istio-existence\n")
	l.print("-----------------------\n")
	_, err = c.getNameSpace(v.istioNamespace)
	if err == nil {
		msg := fmt.Sprintf("Istio cannot be installed because the Istio namespace '%v' is already in use", v.istioNamespace)
		errs = multierror.Append(errs, errors.New(msg))
		l.print(msg + "\n")
	} else {
		l.logAndPrintf("Istio will be installed in the %v namespace.\n", v.istioNamespace)
	}

	l.print("\n")
	l.print("#4. Kubernetes-setup\n")
	l.print("-----------------------\n")
	Resources := []struct {
		namespace string
		group     string
	...
	if createErrors == nil {
		l.logAndPrintf("Can create necessary Kubernetes configurations: %v. \n", strings.Join(resourceNames, ","))
	} else {
		l.logAndPrintf("Can not create necessary Kubernetes configurations: %v. \n", strings.Join(errResourceNames, ","))
	}

	l.print("\n")
	l.print("#5. SideCar-Injector\n")
	l.print("-----------------------\n")
	err = c.checkMutatingWebhook()
	if err != nil {
		l.logAndPrintf("This Kubernetes cluster deployed without MutatingAdmissionWebhook support." +
			"See https://istio.io/docs/setup/kubernetes/additional-setup/sidecar-injection/#automatic-sidecar-injection\n")
	} else {
		l.logAndPrintf("This Kubernetes cluster supports automatic sidecar injection." +
	...
	l.print("-----------------------\n")
	if errs == nil {
		l.logAndPrintf("Install Pre-Check passed! The cluster is ready for Istio installation.\n")
	}
	l.print("\n")
	return errs
