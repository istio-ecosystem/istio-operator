...
func getCRAndNamespaceFromFile(filePath string, config *rest.Config, l *Logger) (customResource string, istioNamespace string, err error) {
	if filePath == "" {
		return "", "", nil
	}

	mergedYAML, err := genProfile(false, filePath, "", "", "", true, config, l)
	if err != nil {
		return "", "", err
	}
